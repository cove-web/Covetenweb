...
import AssignmentModal from './AssignmentModal'; // Import the AssignmentModal component
...
const AcceptedModules = () => {
    ...
    const [isAssignModalOpen, setIsAssignModalOpen] = useState(false); // State for AssignmentModal
    ...
    return (
        <>
                ...
                <tbody className="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800"  style={{backgroundColor:'#e8e8e880'}}>
                    {modules.length ? modules?.map((module: any, index: number) =>
                        <tr key={module?.id} className={"bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-700 dark:text-gray-400"}  style={{backgroundColor:'#e8e8e880'}}>
                            ...
                            <td className="px-4 py-3 text-sm">{module?.forModule?.title || 'N/A'}</td>
                            ...
                            <td className="px-4 py-3 text-sm space-x-2 flex items-center justify-center">
                                ...
                                <div className="relative w-40">
                                    <button
                                        onClick={() => handleAssignClick(module?.id)} // Set the ID and open modal
                                        className="bg-secondary h-full rounded-r block w-full text-sm pr-8 py-1 px-3 text-white leading-tight focus:outline-none dark:bg-darkBg dark:border-darkBorder">
                                        Assign
                                    </button>
                                </div>
                            </td>
                        ...
                        <tr className="bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-700 dark:text-gray-400"  style={{backgroundColor:'#e8e8e880'}}>
                            <td colSpan={5} className="px-4 py-3 text-sm">No modules found</td>
                        </tr>
                    )}
                </tbody>
            ...
            <AssignmentModal
                ...
                currentProject={{ moduleId: currentModuleId, clientEmail: clientId }}
                refetchProjects={getModulesData}
            />
